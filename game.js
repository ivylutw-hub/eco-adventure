...
function updateHomeCheckinCard(){
 ...
 const today=localDateKey();el.textContent=st.checkinHistory&&st.checkinHistory[today]?'今天已簽到':'今天尚未簽到';
}
function renderHeaderNature(){
 const el=document.getElementById('mainHeader');if(!el)return;
 const hour=new Date().getHours();el.classList.toggle('header-night',hour<6||hour>=18);el.classList.toggle('header-day',hour>=6&&hour<18);
}
...
function renderAchievements(){
 ...
 const achievements=[
  {icon:'🌱',name:'第一次答對',desc:'答對第一題，踏出守護地球的第一步。',done:(st.totalCorrect||0)>=1,progress:Math.min(st.totalCorrect||0,1),goal:1},
  {icon:'🌳',name:'100題達成',desc:'累積答對 100 題，長成知識守護樹。',done:(st.totalCorrect||0)>=100,progress:Math.min(st.totalCorrect||0,100),goal:100},
  {icon:'🦦',name:'水獺守護者',desc:'解鎖金門歐亞水獺守護者。',done:currentLevel()>=45||st.avatar==='otter',progress:Math.min(currentLevel(),45),goal:45},
  {icon:'🐎',name:'金門野生動物',desc:'解鎖金門生態傳奇守護者。',done:currentLevel()>=50||['otter','horseshoe'].includes(st.avatar),progress:Math.min(currentLevel(),50),goal:50},
  {icon:'🌎',name:'地球英雄',desc:'完成四大 Stage，成為真正的地球英雄。',done:S.every(isDone),progress:S.filter(isDone).length,goal:4}
 ];
 grid.innerHTML=achievements.map(a=>`<article class="achievement-card ${a.done?'earned':'locked'}"><div class="achievement-icon">${a.icon}</div><div><h3>${a.name}</h3><p>${a.desc}</p><div class="achievement-progress"><i style="width:${Math.min(100,a.progress/a.goal*100)}%"></i></div><small>${a.done?'✅ 已達成':`${a.progress}／${a.goal}`}</small></div></article>`).join('');
}
...
function renderBaseSky(){
  ...
  if(!scene)return;
  const weather=BASE_WEATHERS[baseWeatherIndex]||BASE_WEATHERS[0];
...
function renderBase(){
  ...
  baseScene.innerHTML='<div class="base-sky" aria-hidden="true"></div><div class="base-weather-badge"></div><div class="base-island" aria-hidden="true"><span class="island-water"></span><span class="island-shore"></span><span class="island-grass"></span><span class="island-decor island-duck">🦆</span><span class="island-decor island-butterfly">🦋</span><span class="island-decor island-flowers">🌼🌼</span><span class="island-decor island-trees">🌲🌳🌳</span></div><div class="base-path-layer"></div><div class="base-buildings"></div>';
  ...
  renderBaseSky();
  if(baseWeatherTimer)clearInterval(baseWeatherTimer);baseWeatherTimer=setInterval(()=>{if(!document.getElementById('basePage').classList.contains('hide'))advanceBaseWeather()},30000);
  shop.innerHTML='';[...ITEMS].sort((a,b)=>a.cost-b.cost||a.name.localeCompare(b.name,'zh-Hant')).forEach(it=>{
    ...
    d.innerHTML=`<div class="shop-icon">${it.icon}</div><h4>${it.name}</h4><p>${it.desc}</p><small class="owned-count">目前擁有：${count} 個</small><button onclick="buyItem('${it.id}')">🪙 ${it.cost} 再建設一個</button>`;shop.appendChild(d);
  });
}
function buyItem(id){
...
function renderWeaknessBook(){
 ...
 if(unitSelect){
   ...
   unitSelect.innerHTML='<option value="">全部 Stage／單元</option>'+units.map(key=>{const [sid,u]=key.split('|');const s=S.find(x=>x.id===sid);return `<option value="${key}">${s?s.name:sid}・單元 ${u}</option>`}).join('');
