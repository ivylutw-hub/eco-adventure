...
function page(id){
 ['mapPage','stagePage','quizPage','resultPage','basePage','hallPage','leaderboardPage','profilePage','weaknessPage','checkinPage','adminPage']
...
function addBasePath(event){
  if(!st.baseEditMode||!st.basePathMode||event.target.closest('.base-building,.base-tools'))return;
...
function renderBase(){
  ...
  baseScene.innerHTML='<div class="base-sky" aria-hidden="true"></div><div class="base-weather-badge"></div><div class="base-path-layer"></div><div class="base-buildings"></div><div class="base-tools"></div>';
  baseScene.onclick=addBasePath;
  const buildings=baseScene.querySelector('.base-buildings'),paths=baseScene.querySelector('.base-path-layer'),tools=baseScene.querySelector('.base-tools');
  tools.innerHTML=`<button class="${st.baseEditMode?'active':''}" onclick="event.stopPropagation();toggleBaseEdit()">${st.baseEditMode?'✅ 完成擺設':'✋ 編輯基地'}</button><button class="${st.basePathMode?'active':''}" onclick="event.stopPropagation();toggleBasePath()">🟫 ${st.basePathMode?'停止鋪路':'自己鋪路'}</button><button onclick="event.stopPropagation();clearBasePaths()">🧹 清除路徑</button>`;
