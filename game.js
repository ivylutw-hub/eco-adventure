...
function page(id){
 ['mapPage','stagePage','quizPage','resultPage','basePage','hallPage','leaderboardPage','profilePage','weaknessPage','adminPage']
...
function renderBase(){
  baseCoins.textContent=st.coins;
  baseScene.innerHTML='<div class="base-sky" aria-hidden="true"></div><div class="base-weather-badge"></div><div class="base-buildings"></div>';
  ...
  else st.owned.forEach(id=>{let it=ITEMS.find(x=>x.id===id);if(!it)return;let s=document.createElement('span');s.className='base-building';s.textContent=it.icon;s.title=it.name;buildings.appendChild(s)});
  renderBaseSky();
  if(baseWeatherTimer)clearInterval(baseWeatherTimer);
  baseWeatherTimer=setInterval(()=>{
    if(!document.getElementById('basePage').classList.contains('hide'))advanceBaseWeather();
  },30000);
  // 商店依建設價格由低到高排列；同一建設可重複購買，不設次數限制。
  shop.innerHTML='';[...ITEMS].sort((a,b)=>a.cost-b.cost||a.name.localeCompare(b.name,'zh-Hant')).forEach(it=>{
    const count=st.owned.reduce((n,id)=>n+(id===it.id?1:0),0),d=document.createElement('div');
    d.className='shop-item';
    d.innerHTML=`<div class="shop-icon">${it.icon}</div><h4>${it.name}</h4><p>${it.desc}</p><small class="owned-count">目前擁有：${count} 個</small><button onclick="buyItem('${it.id}')">🪙 ${it.cost} 再建設一個</button>`;
    shop.appendChild(d);
  })
}
function buyItem(id){
  const it=ITEMS.find(x=>x.id===id);if(!it)return;
  if(st.coins<it.cost){toast('金幣不足，完成更多單元再回來建設吧！');return}
  st.coins-=it.cost;st.owned.push(id);save();header();renderBase();toast('✨ 已新增一個「'+it.name+'」！')
}
...
function renderProfile(){
  ...
  AVATARS.forEach(a=>{
    ...
    b.innerHTML=`<span>${avatarMarkup(a,a.name)}</span><b>${a.name}</b><small>${unlocked?'已解鎖':`Lv.${a.level} 解鎖`}</small>`;
