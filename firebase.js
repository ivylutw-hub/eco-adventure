...
let cloudRetryTimer=null, cloudWriteInProgress=false, cloudWriteQueued=false;
...
async function loadCloudPlayer(){
 ...
 try{
  ...
  if(data&&data.state){
   ...
   st={...defaultState(),...st,exp:Number(data.guardianExp)||0,coins:Number(data.coins)||0,unitProgress:data.stageProgress||st.unitProgress||{}};
  }else if(!st.name||st.name==='沄芯'){
