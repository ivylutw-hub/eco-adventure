...
function defaultState(){return{loggedIn:false,name:'沄芯',coins:0,last:'',streak:0,completed:{},lastScores:{},owned:[],savedAt:'',unitProgress:{},unitScores:{},avatar:'fox',frame:'none',exp:0,totalCorrect:0,totalAnswered:0,playDays:0,lastPlayDate:'',soundEnabled:true,wrongNotes:{}}}
...
function page(id){['mapPage','stagePage','quizPage','resultPage','basePage','hallPage','profilePage','weaknessPage'].forEach(x=>document.getElementById(x).classList.add('hide'));document.getElementById(id).classList.remove('hide');scrollTo(0,0)}
...
function renderQ(){
 ...
 selectedAnswer=null;
 const q=quiz[qi];
 quizCount.textContent=`${qi+1}/10`;
 quizBar.style.width=qi*10+'%';
 levelTag.textContent=QUESTION_BANK[stage.main].includes(q)?stage.main:stage.support;
...
function questionNoteKey(q){return `${stage.id}|${q.q}`}
...
function finish(){
 ...
 if(perfect)st.coins=(st.coins||0)+1;
 ...
 coinReward.textContent=perfect?'+1':'未獲得';
