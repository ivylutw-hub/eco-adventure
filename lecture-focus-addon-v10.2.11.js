...
(function(){
  ...
  .lecture100-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:18px;}
  .lecture100-submit{border:0;border-radius:14px;padding:11px 18px;font-weight:800;cursor:pointer;background:#f0a34b;color:#fff;}
  .lecture100-submit:disabled{opacity:.45;cursor:not-allowed;}
  .lecture100-next{min-width:110px;}
  ...
  async function loadQuestions(){
    ...
    try{
      ...
      // Firebase 可能因重複匯入同一份 CSV 而出現同一題多筆資料。
      // 本專區以「題目文字」作為唯一鍵，自動去除重複，並嚴格只保留這 100 題。
      const seen=new Set();
      all=docs.filter(x=>TARGET_SET.has(String(x.q||x.question||''))).filter(x=>{
        const q=String(x.q||x.question||'');
  ...
  function renderQ(){
    ...
    document.getElementById('lecture100Submit').onclick=()=>submitAnswer(q);
  ...
  function choose(i,button,q){
    // 和主線題庫相同：點選選項只是「選擇」，不會立即送出或公布答案。
    if(selected!==null)return;
    selected=i;
    document.querySelectorAll('.lecture100-opt').forEach((b,j)=>{
      b.classList.toggle('selected',j===i);
    });
    const fb=document.getElementById('lecture100Feedback');
    fb.classList.remove('hide');
    fb.innerHTML='<b>📝 已選擇 '+('ABCD'[i])+ '。</b> 請確認後按「確定送出」。';
    document.getElementById('lecture100Submit').disabled=false;
  }
  function submitAnswer(q){
    ...
    const ok=i===q.ans;
    if(ok)correctCount++;
    else wrong.push({n:idx+1,q:q.q,correct:q.opts[q.ans],your:q.opts[i]});
    document.querySelectorAll('.lecture100-opt').forEach((b,j)=>{
      b.disabled=true;
    ...
    fb.classList.remove('hide');
    fb.innerHTML=ok
      ? `<b>✅ 答對了！</b><br>${escapeHtml(q.exp)}`
      : `<b>❌ 再想一下</b><br>正確答案：${'ABCD'[q.ans]}. ${escapeHtml(String(q.opts[q.ans]))}<br>${escapeHtml(q.exp)}`;
    document.getElementById('lecture100Submit').disabled=true;
