...
管理後臺沿用現有 `players/{uid}` 文件，不建立新的玩家資料庫。

首次指定管理者：

1. 先使用要設為管理者的 Google 帳號登入遊戲一次。
2. 到 Firebase Console → Firestore Database → `players`。
3. 找到該帳號 UID 的文件。
4. 手動新增布林欄位：
...
5. 部署本版本的 `firestore.rules`。
6. 登出再重新登入，導覽列會出現「🛠️ 管理後臺」。

普通玩家無法透過遊戲自行把 `isAdmin` 改為 `true`。目前後臺為唯讀資料概況，避免誤刪或誤改玩家進度。

## V9.1.3 金幣獎勵

| 關卡 | 主線單元首次全對 | 單元怪獸弱點全部淨化 |
|---|---:|---:|
| Stage 1 | 2 枚 | 1 枚 |
| Stage 2 | 4 枚 | 2 枚 |
| Stage 3 | 6 枚 | 3 枚 |
...
基地商店會依建設價格由低到高排列。同一種建設可重複購買，購買後會在基地中增加一個相同建設，商店同時顯示目前擁有數量。

## 如何進入管理後臺

1. 先以要設定為管理者的 Google 帳號登入遊戲一次。
2. 進入 Firebase Console → Firestore Database → `players`。
3. 找到該帳號 UID 對應的玩家文件。
4. 新增欄位 `isAdmin`，資料型態選 Boolean，值設為 `true`。
5. 將專案內的 `firestore.rules` 部署到 Firebase。
6. 回到遊戲登出，再重新登入同一個 Google 帳號。
7. 頂端導覽列會出現「🛠️ 管理後臺」，點擊即可進入。

若沒有出現按鈕，請確認 `isAdmin` 是布林值 true，而不是文字字串 "true"，並確認 Firestore Rules 已成功發布。
