...
function authErrorMessage(err){
 ...
 const map={
  'auth/api-key-not-valid.-please-pass-a-valid-api-key.':'Firebase API 金鑰無效或目前無法使用。',
  'auth/unauthorized-domain':'目前網站網域未獲 Firebase Authentication 授權。',
  'auth/network-request-failed':'網路連線或 Firebase Authentication 請求失敗。',
  'auth/popup-blocked':'瀏覽器阻擋了 Google 登入視窗。',
  'auth/popup-closed-by-user':'Google 登入視窗被關閉，登入尚未完成。',
  'auth/cancelled-popup-request':'上一個 Google 登入視窗尚未完成。',
  'auth/operation-not-supported-in-this-environment':'目前瀏覽器環境不支援這種登入方式。',
  'auth/too-many-requests':'登入請求過於頻繁，請稍後再試。',
  'auth/invalid-api-key':'Firebase API 金鑰無效。'
 };
 const friendly=map[code]||'Google 登入未完成。';
 return `${friendly}\n錯誤代碼：${code}${raw?`\n詳細訊息：${raw}`:''}`;
}
function showAuthError(err){
 console.error('Google 登入錯誤',err);
 loginMessage(authErrorMessage(err));
 setGoogleButton(false,'🟢 使用 Google 開始冒險');
}
...
async function initFirebase(){
 ...
 try{
  ...
  await cloudAuth.getRedirectResult().catch(err=>{showAuthError(err);return null;});
...
async function googleLogin(){
 ...
 try{
  ...
  if(['auth/popup-blocked','auth/cancelled-popup-request','auth/operation-not-supported-in-this-environment'].includes(err.code)){
   try{await cloudAuth.signInWithRedirect(provider);return;}catch(redirectErr){showAuthError(redirectErr);return;}
  }
  showAuthError(err);
 }
}
...
async function checkAccountAccess(){
 // 只有「已存在 players 文件」的既有使用者才可進入。
 // 新 Google 帳號即使成功完成 Google 驗證，也不會自動建立玩家帳號。
 // 先讀 players，再讀 blockedUsers；若 blockedUsers 因 Firestore 規則拒絕讀取，
 // 不影響已存在 players 的既有使用者登入。
 try{
  const playerSnap=await playerDoc().get();
  if(!playerSnap.exists){
   return {allowed:false,message:'此網站僅限已授權的既有使用者使用。此 Google 帳號尚未被授權。'};
  }
  try{
   const blockedSnap=await cloudDb.collection('blockedUsers').doc(cloudUser.uid).get();
   if(blockedSnap.exists){
    const data=blockedSnap.data()||{},until=timestampMillis(data.untilAt);
    if(data.status==='deleted')return {allowed:false,message:'此遊戲帳號已由管理者永久刪除。'};
    if(data.status==='suspended'&&(!until||until>Date.now())){
     return {allowed:false,message:until?`此帳號已停用至 ${new Date(until).toLocaleDateString('zh-TW')}。`:'此帳號已停用，請聯絡管理者。'};
    }
   }
  }catch(blockErr){
   if(blockErr&&blockErr.code!=='permission-denied')throw blockErr;
   console.warn('blockedUsers 尚未開放讀取權限；保留既有 players 使用者登入。',blockErr);
  }
  return {allowed:true,playerData:playerSnap.data()||{}};
 }catch(err){
  console.error('使用者授權檢查失敗',err);
  return {allowed:false,message:'目前無法確認使用者授權狀態，為保護網站資料，請稍後再試。'};
 }
}
...
async function loadAdminQuestions(render=true){
 ...
 adminQuestionsCache=snap.docs.map(d=>({id:d.id,...d.data()}));
...
function renderPetQuestionStats(){
 ...
 const counts=keys.map(([stageId,unit,label])=>{
  const all=adminQuestionsCache.filter(q=>q.stageId===stageId&&Number(q.unit)===unit);
  const active=all.filter(q=>q.active!==false);
  return {stageId,unit,label,total:all.length,active:active.length};
 });

 const total=counts.reduce((n,x)=>n+x.total,0);
 const activeTotal=counts.reduce((n,x)=>n+x.active,0);
 ...
 box.innerHTML=`
  <div class="pet-stat-head">
   <div><b>🐾 毛孩守護者題庫統計</b><small>直接統計 Firestore 後台目前讀到的題目</small></div>
   ${status}
  </div>
  <div class="pet-stat-total">
   <strong>${total}</strong><span>總題數</span>
   <strong>${activeTotal}</strong><span>啟用中</span>
  </div>
  <div class="pet-stat-grid">
   ${counts.map(x=>{
    const cls=x.total===50?'ok':x.total>50?'warn':'bad';
    return `<div class="pet-stat-unit ${cls}">
      <small>${x.stageId.toUpperCase()} · UNIT ${x.unit}</small>
      <b>${escapeRankText(x.label)}</b>
      <strong>${x.total} 題</strong>
      <span>${x.active} 題啟用${x.total===50?' · 完整':x.total>50?' · 可能重複':' · 缺 '+(50-x.total)+' 題'}</span>
     </div>`;
   }).join('')}
  </div>`;
}
